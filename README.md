...
| loadMoreTotalCount     | `Number`             | `undefined` | Total number of rows which can be loaded using infinite loader.                 | No       |
| loadMoreRows           | `Function`           | `undefined` | Invoked when a new page is requested when using infinite loader.                | No       |
| loadMoreThreshold      | `Number`             | `50`        | How much in percents to scroll until loadMoreRows is invoked.                   | No       |
| loadMoreLoaderRowCount | `Number`             | `5`         | The number of additional rows to display at the bottom for progress indication. | No       |
| loadMorePageSize       | `Number`             | `30`        | A number of rows loaded with each portion.                                      | No       |
| loadMoreCellRenderer   | `Node` or `Function` | `undefined` | Custom content to dispaly within cells of additional loader rows.               | No       |
